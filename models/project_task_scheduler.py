...
class ProjectTaskNativeScheduler(models.Model):
    ...
    def _ap_calc_tasks_list(self, project_ap, tasks_ap, t_params, revers_step=False):
        ...
        search_tasks, scheduling_type = self._ap_calc_tasks_list_search(t_params=t_params, revers_step=revers_step)

        t_params.update({"scheduling_type" : scheduling_type})

        for stask in search_tasks:
            tasks_ap = self._ap_calc_scheduler_first_work(task=stask, tasks=tasks_ap,project=project_ap,
                                                          t_params=t_params , revers_step=revers_step)
        return tasks_ap

    def _ap_calc_tasks_list_search(self, t_params, revers_step=False):
    ...
    def _ap_cal_period(self, task_obj, direction, new_date, date_type, t_params, cal_value=None):
    ...
    # def _task_val_append(self, task, task_id, val_name, val_value):
    #     if task["id"] == task_id:
    #         task[val_name].append(val_value)
    #
    #     return task
